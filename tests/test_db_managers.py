...
def test_create_subscription(db_manager, subscription_data):
    with db_manager.get_session() as session:
        subscription = db_manager.subscriptions.create_subscription_from_pydantic(subscription_data, session=session)
...
def test_delete_subscription(db_manager, subscription_data):
    with db_manager.get_session() as session:
        subscription = db_manager.subscriptions.create_subscription_from_pydantic(subscription_data, session=session)
        db_manager.subscriptions.delete_subscription_by_id(subscription.id, session=session)
        deleted_subscription = session.query(SubscriptionModel).filter_by(id=subscription.id).first()
        assert deleted_subscription is None

def test_get_subscription_by_id(db_manager, subscription_data):
    with db_manager.get_session() as session:
        subscription = db_manager.subscriptions.create_subscription_from_pydantic(subscription_data, session=session)
        retrieved_subscription = db_manager.subscriptions.get_subscription_by_id(subscription.id, session=session)
        assert subscription.id == retrieved_subscription.id
        retrieved_subscription = db_manager.subscriptions.get_subscription_by_id("invalid id", session=session)
        assert retrieved_subscription is None

def test_create_account(db_manager, account_data):
    with db_manager.get_session() as session:
        account =db_manager.accounts.create_account_from_pydantic(account_data, session=session)
...
def test_delete_account(db_manager, account_data):
    with db_manager.get_session() as session:
        account =db_manager.accounts.create_account_from_pydantic(account_data, session=session)
        db_manager.accounts.delete_account_by_id(account.user_id, session=session)
        deleted_account = session.query(AccountModel).filter_by(token=account.token).first()
...
def test_get_account_by_token(db_manager, account_data):
    with db_manager.get_session() as session:
        account =db_manager.accounts.create_account_from_pydantic(account_data, session=session)
        retrieved_account =db_manager.accounts.get_account_by_token(account.token, session=session)
        assert retrieved_account.user_id == account.user_id
        retrieved_account =db_manager.accounts.get_account_by_token("invalid_token", session=session)
...
def test_reserve_credits(db_manager, account_data):
    with db_manager.get_session() as session:
        account = db_manager.accounts.create_account_from_pydantic(account_data, session=session)
        reservation= db_manager.accounts.reserve_credits(account.user_id, 500, "personal", session=session)
...
def test_deduct_credits_by_reservation_id(db_manager, account_data):
    with db_manager.get_session() as session:
        account =db_manager.accounts.create_account_from_pydantic(account_data, session=session)
        reservation =db_manager.accounts.reserve_credits(account.user_id, 1000, "personal", session=session)
        db_manager.accounts.deduct_credits_by_reservation_id(reservation.id, session=session)
...
def test_cancel_reservation(db_manager, account_subscription_data):
    with db_manager.get_session() as session:
        ...
        db_manager.accounts.cancel_reservation_by_id(reservation.id, session=session)
        canceled_reservation = session.query(CreditsReservationModel).get(reservation.id)
...
def test_calculate_personal_credits(db_manager, account_data, reservations_count):
    with db_manager.get_session() as session:
        ...
        for _ in range(reservations_count):
            reserved_credits += 10
            db_manager.accounts.reserve_credits(account.user_id, 10, "personal", session=session)
        total = db_manager.accounts.calculate_personal_credits(account.token, session=session)
...
def test_calculate_all_account_subscription_credits(db_manager):
    with db_manager.get_session() as session:
        ...
        session.add(account)
        session.commit()
        session.refresh(subscription1)
        session.refresh(subscription2)
        ...
        acc_subscription2 = AccountSubscriptionModel(user_id="user1", subscription_id=subscription2.id,
                                                     credits=20,
                                                     next_renewal=(datetime.now() + timedelta(days=30)))
        session.add(acc_subscription1)
        session.add(acc_subscription2)
        session.commit()
        expected_credits_sum = acc_subscription1.credits + acc_subscription2.credits

        result = db_manager.subscriptions.calculate_all_account_subscription_credits("user1", session=session)

        # Проверка на соответствие ожидаемому результату
        assert result == expected_credits_sum
        result = db_manager.subscriptions.calculate_all_account_subscription_credits_by_token("token1", session=session)
        assert result == expected_credits_sum
